...
export default function Profile() {    
    ...
    const fetchUserData = (id: any, token: any) => {
        mfaapi.get(`api/getuserbyid/${id}`,{headers: {
        ...
        .then((res: any) => {
            ...
            const userpic: string = `http://localhost:5000/assets/users/${res.data.userpicture}`;
            setUserpicture(userpic);
            setQrcodeurl(res.data.qrcodeurl);     

            if (res.data.qrcodeurl !== null) {
                const qrcode: string = 'data:image/png;base64,' + res.data.qrcodeurl
                setQrcodeurl(qrcode);
    ...
    const changePicture = async (event: any) => {
        event.preventDefault();
            ...
            await api.patch(`api/uploadpicture/${userid}`, formData, {headers: {
                Authorization: `Bearer ${token}`
            }})
            .then((res: any) => {
                setProfileMsg(res.data.message);
                setTimeout(() => {
                    const userpic: string = `http://localhost:5000/assets/users/${res.data.userpicpicture}`;
    ...
    return (
      <div className='profile-bg'>
        ...
        <div className="card-header bg-primary">
            <h3 className="text-white">User Profile ID No. <span className="id-size">{userid}</span></h3>
        </div>
